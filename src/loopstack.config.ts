...
export const appConfig: ReturnType<typeof registerAs<AppConfig>> =
  registerAs<AppConfig>('app', () => ({
    nodeEnv: process.env.NODE_ENV || 'development',
    isLocalMode: process.env.ENABLE_LOCAL_MODE === 'true',
  }));

export const authConfig: ReturnType<typeof registerAs<AuthConfig>> =
  registerAs<AuthConfig>('auth', () => {
    const isProduction = process.env.NODE_ENV === 'production';

    if (isProduction) {
      if (!process.env.CLIENT_ID || !process.env.CLIENT_SECRET) {
        throw new Error(
          'CLIENT_ID and CLIENT_SECRET are required in production',
        );
      }
    }
